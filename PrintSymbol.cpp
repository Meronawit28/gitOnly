...
const wchar_t * const rgTags[] =
{
  ...
  L"TaggedUnionCase",                  // SymTagTaggedUnionCase
};
...
void PrintDiaTagValue(DiaTagValue *value) {
  switch (value->valueSizeBytes) {
    case 0:
      ...
      break;
    case 1:
      wprintf(L"0x%X", *((UINT8 *)&value->value));
      break;
    case 2:
      wprintf(L"0x%X", *((UINT16 *)&value->value));
      break;
    case 4:
      wprintf(L"0x%I32X", *((UINT32 *)&value->value));
      break;
    case 8:
      wprintf(L"0x%I64X", *((UINT64 *)&value->value));
      break;
    case 16: {
...
void PrintTypeInDetail(IDiaSymbol *pSymbol, DWORD dwIndent)
{
  ...
  switch (dwSymTag) {
    ...
    case SymTagTaggedUnionCase: {
      ...
      if (SUCCEEDED(pSymbol->QueryInterface(&pSymbol11)) && pSymbol11->get_discriminatedUnionTag(&pTagType, &tagOffset, &mask) == S_OK) {
        wprintf(L"\nTag            :   ");
        PrintIndent(dwIndent);
        wprintf(L"this+0x%X, Type: ", tagOffset);
        PrintType(pTagType);
